 ...
 *
 * Z          X         C        V         B
 * transparent scroll   snipe    DPI+      DPI-
 *
 * N          M         ,        .         -
 * snipeDPI+  snipeDPI- snipe    scroll    transparent
 */
...
/*
 * Numerals - adapted for Swedish OS layout.
 */
...
/*
 * Symbols - adapted for Swedish OS layout.
 *
 * Å = KC_LBRC
 * Ö = KC_SCLN
 */
...
/*
 * Hold physical Z or the right-most bottom key to enter pointer layer.
 */
#define _POINTER_MOD(                                                 \
    L00, L01, L02, L03, L04, R05, R06, R07, R08, R09,               \
    L10, L11, L12, L13, L14, R15, R16, R17, R18, R19,               \
...
/*
 * Automatically enable sniping/precision mode whenever the pointer
 * layer is active.
 *
 * This replaces the old:
 * charybdis_set_pointer_sniping_enabled(...)
 *
 * with BastardKB's current bk_pointing_device API.
 */
#ifdef POINTING_DEVICE_ENABLE

layer_state_t layer_state_set_user(layer_state_t state) {
    bkpd_set_pointer_sniping_enabled(
        layer_state_cmp(state, LAYER_POINTER)
    );
